...
时，如果 AI 使用原生 Anthropic `/v1/messages` 协议且
`rss.anthropic_web_fetch_enabled=true`，同一次 digest 请求会尝试一次受域名限制的
Web Fetch（页面内容上限 12,000 token）。工具失败后回退到 RSS 摘要，再回退到 digest
预览，不阻断消化和租户投递。可单独关闭 Web Fetch；关闭
`rss.fetch_original_abstract` 会同时关闭 Python 抓取和 Web Fetch 原文增强。
