...
时，如果 AI 使用官方智谱通用 API（`https://open.bigmodel.cn/api/paas/v4`）和 GLM
模型，且 `rss.glm_reader_enabled=true`，后端会调用一次智谱 Reader，最多将 48,000
字符的页面内容在内存中交给当前 GLM 生成 digest，不保存完整网页。Reader 失败后
回退到 RSS 摘要，再回退到 digest 预览，不阻断消化和租户投递。关闭
`rss.fetch_original_abstract` 会同时关闭 Python 和智谱 Reader 原文增强。
